/*********Brute Force*********/
int LCSrecursion(string s,string t)
{
    //base case
    ...
    //recursive calls
    if(s[0]==t[0]) 
      return 1+LCSrecursion(s.substr(1),t.substr(1));
    else{
        int a=LCSrecursion(s.substr(1),t);
        int b=LCSrecursion(s,t.substr(1));
        int c=LCSrecursion(s,t.substr(1));
        return max(a,b);
    }
}
